...
def maxMin_to_lattice(maxMin):
    maxVal = maxMin[0]
    minVal = maxMin[1]
    maxOnLattice = value_to_lattice(maxVal,0)
    if onLattice(minVal):
        minOnLattice = value_to_lattice(minVal,0)
    else:
        minOnLattice = value_to_lattice(minVal,1)
...
def build_lattice_slice(maxMin, xValue):
    ...
    latticeYSlice = [minVal + (LATTICE_SIZE * i) for i in range(0, ySteps)]
...
def generate_lattice(polygon):
    ...
    for xValue in xLattice:
        ...
        roughMaxMin = get_maxMin(intersections)
        maxMin = maxMin_to_lattice(roughMaxMin)
        print(maxMin)        
