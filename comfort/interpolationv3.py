...
def Points_to_Coeffs(tht_i_phi_i, n):
	...
	G = bc.process(t, p, v, n, 0)
	list_of_coeffs = []
	for i in range(len(G)):
		list_of_coeffs += quint.interp(G[i])
	return [list_of_coeffs, t]

def Coeffs_to_Vals(a, t_i):
	s = np.linspace(t_i[0]+.05,t_i[-1]-.05, 500)
...
a, t = Points_to_Coeffs(tht_i_phi_i, 6)
x_plot = Coeffs_to_Vals(a, t)
...
plt.plot(t_plot, x_plot, t, x_i)#, y, z,c='b',marker='+')
